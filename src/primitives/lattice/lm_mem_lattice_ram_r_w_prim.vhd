...
--=============================================================================
-- Module Name : lm_mem_lattice_ram_r_w_prim
-- Library     : -
-- Project     : lm_memories
-- Company     : LogiMentor Srl
-------------------------------------------------------------------------------
-- Description:
--  Lattice primitive-ready RAM wrapper.
...
entity lm_mem_lattice_ram_r_w_prim is
  generic(
    g_ram_latency : positive := 1;
    g_ram_data_w  : positive := 32;
    g_ram_depth   : positive := 1024;
    g_family      : string   := "ecp5";
    g_primitive   : string   := "ebr"
  );
  port(
    clk_i     : in  std_logic;
...
end entity lm_mem_lattice_ram_r_w_prim;

architecture a_rtl of lm_mem_lattice_ram_r_w_prim is
begin
  assert (g_family'length > 0) and (g_primitive'length > 0)
    report "lm_mem_lattice_ram_r_w_prim: family and primitive must be set"
    severity failure;
